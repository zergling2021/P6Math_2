...
from .models import ScienceChoice, ScienceQuestion, ScienceTopic, ScienceSubTopic, ScienceLevel3Topic, AnswerRecords
...
class IndexView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        """
        print("user logged in is : ", self.request.user)
...
class ScienceReportView(LoginRequiredMixin, generic.ListView):
...
class ScienceMCQView(LoginRequiredMixin, generic.ListView):
...
class ScienceOpenView(LoginRequiredMixin, generic.ListView):
...
class ScienceReportresultView(LoginRequiredMixin, generic.ListView):
...
def vote(request, question_id):
  ...
  #  except (KeyError, ScienceChoice.DoesNotExist):
   ...
   #     return render(request, 'science/detail.html', {
    ...
    newAnswerRecord = AnswerRecords()
    newAnswerRecord.record_date =  datetime.datetime.now()
    #newAnswerRecord.record_date = datetime.datetime.now()
    if 1:
        ...
        newAnswerRecord.attempted_answer_a = attempted_answer_a
        newAnswerRecord.attempted_answer_b = attempted_answer_b
        newAnswerRecord.attempted_answer_c = attempted_answer_c
        ...
        if(question.question_type == "MCQ"):
            if question.correct_answer_a == attempted_answer_a and question.correct_answer_b == attempted_answer_b and question.correct_answer_c == attempted_answer_c:
                ...
                question.number_of_attempts = question.number_of_attempts + 1
                newAnswerRecord.mcq_answer_successful = "No"
            ...
            newAnswerRecord.self_rated_score = self_rated_score
        ...
        newAnswerRecord.user_id = request.user
        newAnswerRecord.question_model_index = question.question_model_index
        newAnswerRecord.topic_number = question.topic_number
        newAnswerRecord.subtopic_number = question.subtopic_number
        newAnswerRecord.lv3_topic_number = question.lv3_topic_number
...
def generate_science_report(request):
    ...
    for record in AnswerRecords.objects.all():
        print("User ID: ", record.user_id)
        print("Question Index: ", record.question_model_index)
        print("Successful? ", record.mcq_answer_successful)
